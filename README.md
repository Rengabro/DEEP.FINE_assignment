...
> 지도에서 직접 추가한 POI도 `tb_poi`에 저장됩니다. 이후 엑셀 Import를 실행하면 기존 정책에 따라 직접 추가한 POI를 포함한 모든 기존 데이터가 삭제되고, 엑셀 데이터로 현행화됩니다.
...
1. 좌측 하단의 `POI 추가` 버튼을 누릅니다.
2. 십자 모양 커서가 표시되면 지도에서 원하는 위치를 클릭합니다.
3. 추가할 POI 이름을 입력합니다.
4. 클릭한 위도/경도와 이름이 DB에 저장됩니다.
5. 새 마커가 지도에 표시되고 선택 상태로 열립니다.

`POI 추가` 모드에서 `Esc` 키를 누르면 저장하지 않고 모드를 취소합니다.
...
제 지도에 저장해 둔 용산 근처 맛집 목록을 공유드려봅니다!

- [맛집 POI 목록 엑셀 파일](./tb_poi_hot_place.xlsx)
